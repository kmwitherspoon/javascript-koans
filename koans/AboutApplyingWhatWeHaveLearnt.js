...
describe("About Applying What We Have Learnt", function() {
    ...
    beforeEach(function() {
        products = [{
            ...
            ingredients: ["roma", "sundried tomatoes",
                "goats cheese", "rosemary"
            ],
            containsNuts: false
        }, {
            name: "South Of The Border",
            ingredients: ["black beans", "jalapenos",
                "mushrooms"
            ],
            containsNuts: false
        }, {
            name: "Blue Moon",
            ingredients: ["blue cheese", "garlic",
                "walnuts"
            ],
            containsNuts: true
        }, {
            name: "Taste Of Athens",
            ingredients: ["spinach", "kalamata olives",
                "sesame seeds"
            ],
            containsNuts: true
        }];
            ...
            for (i = 0; i < products.length; i += 1) {
                if (products[i].containsNuts === false) {
                    ...
                    for (j = 0; j < products[i].ingredients.length; j +=
                        1) {
                        if (products[i].ingredients[j] ===
                            "mushrooms") {
                            hasMushrooms = true;
                        }
                    }
                    if (!hasMushrooms) productsICanEat.push(
                        products[i]);
                }
            }
    ...
    it(
        ...
        function() {
            ...
            var productsICanEat = _.filter(products, function(el) {
                return el.containsNuts === false && !_.contains(
                    el.ingredients, "mushrooms")
            })
            expect(productsICanEat.length).toBe(1);
        });
    /*********************************************************************************/
    ...
    it(
        ...
        function() {
            ...
            _.range(1, 1001).map(function(el) {
                if (el % 3 === 0 || el % 5 === 0) {
                    return sum += el;
                }
            })
        ...
        for (i = 0; i < products.length; i += 1) {
            for (j = 0; j < products[i].ingredients.length; j +=
                ...
                ingredientCount[products[i].ingredients[j]] = (
                    ingredientCount[products[i].ingredients[
                        j]] || 0) + 1;
            }
        }
    ...
    it("should count the ingredient occurrence (functional)", function() {
        ...
        var ingredientList = _.map(products, function(el) {
            return el.ingredients;
        });
        var flatIngredients = _.flatten(ingredientList);
        _.reduce(flatIngredients, function(sum, el) {
